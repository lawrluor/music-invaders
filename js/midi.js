  ...
  updateDeviceList() {
    ...
    if (count === 0) {
      ...
      if (lastDeviceId) {
        ...
        if (this.selectInput(lastDeviceId)) return;  // successfully loaded last used device
      }

      // If last used device not found, or failed to automatically select it, select first available
      this.deviceSelect.value = this.midiInputs[0].id;
      if (this.selectInput(this.midiInputs[0].id)) return;  // successfully loaded first available device
